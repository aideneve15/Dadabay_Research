...
X <- as.matrix(peak_wide[, 2:4])  # first 3 peaks
Y <- as.matrix(peak_wide[, 5:8]) 
...
U <- as.matrix(X) %*% cca_result$xcoef
V <- as.matrix(Y) %*% cca_result$ycoef


plot_df <- data.frame(
  U1 = U[,1],
  V1 = V[,1],
  sample = peak_wide$sample
)

# Scatter plot
ggplot(plot_df, aes(x = U1, y = V1, label = sample)) +
  geom_point(color = "blue", size = 3) +
  geom_text(nudge_y = 0.05) +
  theme_minimal() +
  labs(
    title = "First Canonical Variates: X vs Y",
    x = "U1 (X canonical variate)",
    y = "V1 (Y canonical variate)"
  )


xcoef_df <- data.frame(
  peak_id = colnames(X),
  coef = cca_result$xcoef[,1]  # first canonical variate
) |> 
...
ycoef_df <- data.frame(
  peak_id = colnames(Y),
  coef = cca_result$ycoef[,1]
) |> 
  mutate(abs_coef = abs(coef)) |> 
  arrange(desc(abs_coef))


p1 <- ggplot(xcoef_df, aes(x = reorder(peak_id, abs_coef), y = coef)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() +
  labs(
    title = "X Peaks → U1",
    x = "Peak Number",
    y = "Canonical Coefficient"
  ) +
  theme_minimal()

# Y peaks plot
p2 <- ggplot(ycoef_df, aes(x = reorder(peak_id, abs_coef), y = coef)) +

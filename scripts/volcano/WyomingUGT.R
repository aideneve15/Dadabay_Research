...
"
wyoming_peak_areas_UGT <- wyoming_peak_areas |> 
  dplyr::filter(enzyme == 'UGT')

library(dplyr)
library(broom)
library(purrr)

wyoming_volcano_stats <- wyoming_peak_areas_UGT |> 
...
p <- ggplot(data = plot_df, 
  ...
  labs(
    title = "Wyoming Sagebrush with UGT Enzyme",
...
ggsave('./images/volcano/WyomingUGT.png', p, width = 6, height = 4, units = 'in', dpi = 600)

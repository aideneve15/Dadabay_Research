...
"
wyoming_peak_areas_Com <- wyoming_peak_areas |> 
  dplyr::filter(enzyme == 'CYP+UGT')

library(dplyr)
library(broom)
library(purrr)

wyoming_volcano_stats <- wyoming_peak_areas_Com |> 
  group_by(enzyme, species, peak_number, time_point) |> 
  filter(time_point != 0) |> 
  group_modify(~{
    
    # extract baseline (time_point == 0) for same enzyme/species/peak_number
    baseline <- wyoming_peak_areas |> 
      filter(
        enzyme == .y$enzyme,
        species == .y$species,
...
wyoming_peak_areas_with_stats <- wyoming_peak_areas |> 
  left_join(
    wyoming_volcano_stats,
    by = c("enzyme", "species", "peak_number", "time_point")
  ) |> 
  mutate(fc = ifelse(time_point == 0, 1, fc))

wyoming_peak_areas_with_stats <- wyoming_peak_areas_with_stats |> 
...
wyoming_peak_areas_with_stats$diffexpressed <- "Insignificant"
wyoming_peak_areas_with_stats$diffexpressed[wyoming_peak_areas_with_stats$log2fc > 0.4 & wyoming_peak_areas_with_stats$p_value < 0.05] <- "Up Regulated"
wyoming_peak_areas_with_stats$diffexpressed[wyoming_peak_areas_with_stats$log2fc < -0.4 & wyoming_peak_areas_with_stats$p_value < 0.05] <- "Down Regulated"

mycolors <- c("blue", "red", "grey")
names(mycolors) <- c("Down Regulated", "Up Regulated", "Insignificant")

wyoming_peak_areas_with_stats$delabel <- NA
wyoming_peak_areas_with_stats$delabel[wyoming_peak_areas_with_stats$diffexpressed != "Insignificant"] <- wyoming_peak_areas_with_stats$rt_label[wyoming_peak_areas_with_stats$diffexpressed != "Insignificant"]

plot_df <- wyoming_peak_areas_with_stats |>
...
p <- ggplot(data = plot_df, 
  ...
  labs(
    title = "Wyoming Sagebrush with CYP+UGT Enzymes",
...
ggsave('./images/volcano/WyomingCom.png', p, width = 6, height = 4, units = 'in', dpi = 600)

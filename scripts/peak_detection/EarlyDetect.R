peak_ranges <- function(df, min_height = 0.15, min_time_distance = 0.1, expand_frac = 0.05) {
  library(pracma)
  library(dplyr)
  library(tibble)
...
Early <- allLong |> 
  dplyr::filter(species == 'early', enzyme == 'CYP') |> 
  normalize_byspecies() |> 
...
EarlyCYP_peaks <- collapsed_peak_ranges 


Early <- allLong |> 
  dplyr::filter(species == 'early', enzyme == 'UGT') |> 
  normalize_byspecies() |> 
  mutate(time_point = factor(time_point, levels = c(0,2,5,15,30))) |> 
  drop_na()
...
# filter for plotting just m4
EarlyUGT_peaks <- collapsed_peak_ranges 


Early <- allLong |> 
  dplyr::filter(species == 'early', enzyme == 'CYP+UGT') |> 
  normalize_byspecies() |> 
  mutate(time_point = factor(time_point, levels = c(0,2,5,15,30))) |> 
  drop_na()
...
EarlyCom_peaks <- collapsed_peak_ranges |> 
  dplyr::filter(start_time > 0.8) 


p <- ggplot(Early, aes(time, intensity)) +
  geom_line(alpha = 0.6) +
  geom_rect(
    data = EarlyCom_peaks,
...
ggsave(
  filename = "images/Peaks_Early.png",
  plot = p,
    dpi = 300,
    width = 8,
    height = 6,
